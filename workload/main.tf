...
resource "null_resource" "wait_for_db" {
  ...
  provisioner "local-exec" {
    command = "sleep 5 && bash wait-for-nomad-job.sh ${nomad_job.mongodb.id} ${data.terraform_remote_state.nomad_cluster.outputs.nomad_public_endpoint} ${data.vault_kv_secret_v2.bootstrap.data["SecretID"]}"
  }
}
